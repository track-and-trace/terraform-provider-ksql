EXECUTABLE=terraform-provider-ksql
...
WINDOWS=$(EXECUTABLE)_$(VERSION).exe
LINUX=$(EXECUTABLE)_$(VERSION)
...
$(WINDOWS):
	...
	env GOOS=windows GOARCH=amd64 go build -v -o $(WINDOWS) -ldflags="-s -w -X main.version=$(VERSION)"  ./*.go
...
$(LINUX):
	...
	env GOOS=linux GOARCH=amd64 go build -v -o $(LINUX) -ldflags="-s -w -X main.version=$(VERSION)"  ./*.go
...
$(DARWIN):
	...
	env GOOS=darwin GOARCH=amd64 go build -v -o $(DARWIN) -ldflags="-s -w -X main.version=$(VERSION)"  ./*.go
